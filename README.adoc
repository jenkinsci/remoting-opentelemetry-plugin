...
==== 2. Build monitoring-engine sub module

[source,console]
....
$ mvn package -DskipTests -pl monitoring-engine
....

This wil generate `remoting-opentelemetry-engine.jar` at `monitoring-engine/target/`
...
$ java \
-javaagent ./path/to/remoting-opentelemetry-engine.jar \
-jar agent.jar \
-jnlpUrl <jnlp url> \
